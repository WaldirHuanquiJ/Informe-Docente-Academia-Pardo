...
class DashboardView:
    ...
    def __init__(self, data_dir: Path) -> None:
        ...
        self.card_total_assigned = QLabel("00:00")
        self.card_total_debt = QLabel("00:00")
        ...
        big_cards = [
            ...
            ("Cobertura Neta", self.card_teacher_rate, 1),
        ...
        card_layout.addWidget(_make_card("Horas Asignadas", self.card_total_assigned, 0, 62), 0, 6, 1, 1)
        card_layout.addWidget(_make_card("Horas Deuda", self.card_total_debt, 3, 62), 1, 6, 1, 1)
    ...
    def _build_day_slot_values(self, teacher: TeacherRecord, day: int) -> list[str]:
        ...
        try:
            ...
            return [""] * len(TIME_SLOTS)
        matrix = self._reporte_engine._build_day_slot_values(teacher)
        if not (0 <= day_idx < len(matrix)):
            return [""] * len(TIME_SLOTS)
        return matrix[day_idx]
    ...
    def _day_total_seconds(self, teacher: TeacherRecord, day: int) -> int:
        ...
        for idx in range(len(day_slots)):
            if idx in scheduled_slots:
                total += self._effective_block_seconds(day_slots[idx], idx)
            else:
                total += compute_seconds_from_mark_text(day_slots[idx])
        return total

    def _effective_block_seconds(self, block_text: str, slot_idx: int) -> int:
        if not (0 <= slot_idx < len(SLOT_START_MINUTES)):
    ...
    def _refresh_teacher_panel(self) -> None:
        ...
        tardiness_pct = self._percent_value(tardiness_seconds, attendance_base, force_visible=True)
        ...
        self.card_total_assigned.setText(format_hm(expected_seconds))
        self.card_total_debt.setText(format_hm(debt_seconds))
        ...
        for row_idx, (day, seconds) in enumerate(rows):
            ...
            expected_day = self._teacher_expected_seconds(teacher, [day])
            perf = min(int((seconds / expected_day) * 100), 100) if expected_day > 0 else 0
    ...
    def _report_column_metrics(self, teacher: TeacherRecord, days: list[int]) -> dict[str, int]:
        # Usar el mismo motor de Reporte para garantizar coincidencia 1:1.
            ...
            for idx, value in enumerate(slots):
                ...
                if idx in scheduled_slots:
                    ...
                    if self._reporte_engine._is_non_working_day(day):
                        continue
                    # Debe seguir exactamente la columna "Total Asistencia" del Reporte.
    ...
    def clear_all(self) -> None:
        ...
        self.card_total_assigned.setText("00:00")
        self.card_total_debt.setText("00:00")
